...
- `plotstyle.py`: shared matplotlib styling for the committed figures.
...
- `run_experiment.py`: static demo, with the LogReg-on-components baseline. Prints a
  results table and writes `separation.png`.
- `run_sieve.py`: sieve demo. Prints a results table (no figure).
